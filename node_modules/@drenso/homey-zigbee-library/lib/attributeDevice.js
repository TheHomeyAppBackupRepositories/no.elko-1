...
async function initReadCommandCapability(device, zclNode, capabilityId, cluster, commandName, commandArgParser = defaultSetParser, attributeName, reportParser = defaultReportParser, minChange, endpointId, maxInterval, pollInterval) {
    const endpoint = endpointId ?? device.getClusterEndpoint(cluster) ?? 1;
    // Retrieve initial value
    await readInitialValue(device, zclNode, capabilityId, cluster, attributeName, reportParser, endpoint);
